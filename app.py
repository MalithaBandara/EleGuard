...
def load_config():
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            st.warning(f"Error loading config.json, using defaults: {e}")
...
INFERENCE_INTERVAL   = config.get("inference_interval", 5.0)    # minimum seconds between queuing a frame per camera
MIN_OBJECT_AREA_FRACTION = config.get("min_object_area_fraction", 0.010)
PERSISTENCE_REQUIRED = config.get("persistence_required", 5)
MAX_FG_RATIO         = config.get("max_fg_ratio", 0.35)
MAX_SCATTER_CONTOURS = config.get("max_scatter_contours", 5)
MORPH_KERNEL = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
AUDIO_THRESHOLD      = config.get("audio_threshold", 0.15)
ALERT_COOLDOWN       = config.get("alert_cooldown", 30.0)
...
def run_inference(image, mode="video"):
    ...
    """
    res_w, res_h = config.get("inference_resolution", [224, 224])
    small = cv2.resize(image, (res_w, res_h))
    ...
    model_url = config.get("model_url", "http://127.0.0.1:8080/v1/chat/completions")
